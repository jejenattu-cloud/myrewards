...
const Sidebar: React.FC<SidebarProps> = ({ currentView, setCurrentView, businessName }) => {
  ...
  return (
    <aside className="w-64 h-screen border-r border-border-color bg-background-light flex flex-col justify-between shrink-0 sticky top-0">
      <div className="p-6">
        <div className="flex items-center gap-3 mb-10 group cursor-pointer" onClick={() => setCurrentView(View.DASHBOARD)}>
          ...
          <div>
            <h1 className="text-lg font-bold leading-tight truncate w-32">{businessName}</h1>
            <p className="text-text-secondary text-xs font-medium">Owner Admin</p>
          </div>
        </div>
      ...
      <div className="p-6 border-t border-border-color">
        <button className="flex w-full items-center justify-center gap-2 rounded-xl h-12 px-4 bg-gray-200 text-text-main text-sm font-bold hover:bg-gray-300 transition-colors">
